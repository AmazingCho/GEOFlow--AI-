...
| 上游选择性采纳（2026-07-11） | 阶段 1/3 已完成 | 已修复已登录管理员入口跳转，并统一默认前台、三套主题、模板工厂与目标站点包 SEO metadata；远端设置同步预览和 Docker Vite 构建分别因现有安全边界、无运行时消费者而跳过 |
...
- `AdminCrmPagesTest` 询盘转商机事务编排、待办/单据原位关联及双向待办来源验证，共 21 tests / 185 assertions
...
- `crm:pipeline-audit --apply` 真实修复：补关联 1 个待办、3 份单据、3 条活动，修复 1 个单据 Collection；审计问题 16 -> 8
