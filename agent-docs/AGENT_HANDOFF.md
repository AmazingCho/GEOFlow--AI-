...
- PI 银行信息独立末页，页码按明细页总数动态顺延；CI+PL 无图片，Quotation+PI 有图片
...
- 单据详情页和打印预览页已提供“下载 PDF”；PDF 使用 `CrmDocumentPdfService` + `scripts/render-crm-document-pdf.mjs` 调用 Chromium/Puppeteer 输出 A4。
- 打印模板支持动态明细分页：第一页显示抬头/买方/前 N 条明细，续页显示 Items continued，末页显示剩余明细、合计、条款和签名。
...
- CRM 销售链路 V2 阶段 0-7 已完成：数据契约、商机来源模式、归档恢复、询盘转商机事务编排、统一活动时间线、单据链一致性、历史唯一候选修复和 UI/文档收口。真实数据审计从 16 项降至 8 项，剩余 1 个孤立商机、5 份无唯一候选单据、2 条无唯一候选活动，必须人工判断。
...
- 2026-06-16 GEO 生成质检与 RAG 解释阶段 1：文章质量评分已输出细化 metrics / reasons；文章编辑页“生成来源”会展示 RAG 策略、使用知识库、召回来源、片段元数据、证据分和匹配原因；`CaseRecord::entity()` 关系已补齐以兼容历史 Case RAG 过滤。
- 2026-06-16 AI 知识库纠错助手阶段 3：知识库详情页和文章编辑页可发起纠错；AI 只生成 correction proposal，管理员批准/拒绝/应用；应用后更新目标 chunk 和知识库正文对应片段，刷新单个 chunk embedding / fallback vector，并写入版本记录支持回滚。
- 2026-06-16 知识库向量化异步队列阶段 4：知识库切片/向量化任务新增 queued/running/completed/failed 状态字段；队列 Job 会写回开始、完成、失败时间和错误信息；列表页与详情页显示状态并轮询，手动更新切片有重复投递保护，失败后可重试。
- 2026-06-16 知识库重复与冲突检测阶段 5：知识库列表新增“治理检查”入口，`/admin/knowledge-bases/governance` 可按 Collection 扫描疑似重复资料和数值型事实冲突；本阶段只读报告，不自动合并、删除、覆盖或重建切片。
- 2026-06-17 主线 Phase 1 任务回收站：任务删除改为软删除并进入回收站；已生成文章和 `articles.task_id` 保留，文章列表/编辑页显示“已删除任务”来源状态；恢复任务后保持暂停，避免自动继续生成。
- 2026-06-17 主线 Phase 2 性能压测补强：确认标签选择器远程搜索、标签引用懒加载和知识库队列状态已有；补齐 tag search `pagination.has_more`、版本化标签统计缓存 key 和 `AdminPerformanceSurfaceTest`。
...
- 2026-07-11 上游 `f603830` 选择性采纳已完成阶段 1/3：登录入口重定向与统一 SEO metadata 已落地。阶段 2 因远端站点设置同步已按产品要求禁用而跳过，阶段 4 因无 Vite 运行时消费者而跳过；阶段 5-7 等用户明确指令。禁止恢复在线主题源码编辑、首页模块体系、远端设置同步或 PostgreSQL volume 目标路径补丁。
...
- 任务回收站已完成核心功能；后续如需永久删除任务，必须先确认数据保留策略，默认不开放物理删除。
- AI 知识库纠错助手已完成核心闭环；当前详情页是并排原文/建议对比，不是字符级 diff，后续可增强 diff 视觉。
- 大数据量下的标签远程搜索、懒加载、统计缓存已完成核心验证；后续重点是真实大数据压测、更多统计入口缓存覆盖，以及用真实大文件压测队列 worker、模型限流和失败重试体验。
...
- CRM 销售链路 V2 主线已收口；后续不要再自动修复剩余 8 项历史歧义数据，除非用户明确指定关联关系或先实现人工治理 UI。
...
以下内容是历史交接记录，仅用于排查旧问题和理解背景；不要把已标注完成或已回滚的历史段落当作当前待执行计划。当前状态以本文件上方摘要、[IMPLEMENTATION_STATUS.md](./IMPLEMENTATION_STATUS.md) 和 [KNOWN_ISSUES.md](./KNOWN_ISSUES.md) 为准。
...
- 单据转换后台能力仍保留在 `crm_quotes` / `source_quote_id`，但前端“创建副本”入口已隐藏；默认工作流通过打印预览 `?type=` 临时输出报价单、PI、CI、装箱单或合同，避免列表产生大量重复单据。
