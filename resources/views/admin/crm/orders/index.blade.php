                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @foreach ($orders as $order)
                                <tr>
                                    <td class="px-6 py-4"><div class="font-semibold text-gray-900">{{ $order->order_no }}</div><div class="mt-1 text-sm text-gray-600">{{ $order->title }}</div><div class="mt-1 text-xs text-gray-500">{{ $order->customer?->company_name ?? '未关联客户' }} · {{ $order->collection?->name ?? '未指定' }} · 负责人：{{ $order->owner ?: '未指定' }}</div></td>
