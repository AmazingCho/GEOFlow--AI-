                        ...
                        @foreach ($models as $model)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ...
                                    @endif
                                    @if (($supportsModelMaxTokens ?? false) && $model['model_type'] === 'chat')
                                        <div class="mt-1 text-xs text-gray-500">
                                            {{ (int) ($model['max_tokens'] ?? 0) > 0
                                                ? __('admin.ai_models.max_tokens_configured', ['tokens' => (int) $model['max_tokens']])
                                                : __('admin.ai_models.max_tokens_default', ['tokens' => (int) ($contentMaxTokens ?? 8192)]) }}
                                        </div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <form id="modelForm" method="POST" action="{{ route('admin.ai-models.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div id="maxTokensField" class="{{ ($supportsModelMaxTokens ?? false) ? '' : 'hidden' }}">
                            ...
                            <input type="number" name="max_tokens" id="max_tokens" min="1" max="200000" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="{{ __('admin.ai_models.max_tokens_placeholder', ['tokens' => (int) ($contentMaxTokens ?? 8192)]) }}">
...
@push('scripts')
    <script>
        ...
        function syncMaxTokensField() {
            const field = document.getElementById('maxTokensField');
            const input = document.getElementById('max_tokens');
            const type = document.getElementById('model_type').value || 'chat';
            if (!field || !input) {
                return;
            }

            if (!SUPPORTS_MODEL_MAX_TOKENS || type !== 'chat') {
        ...
        document.getElementById('model_type').addEventListener('change', syncMaxTokensField);
