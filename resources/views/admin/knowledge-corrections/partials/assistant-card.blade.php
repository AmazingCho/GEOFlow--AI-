...
<div class="rounded-xl border {{ $toneClasses['card'] }} p-5" data-knowledge-correction-card>
    ...
    @endif
        <input type="hidden" name="source_type" value="{{ $sourceType }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif>
        @if($knowledgeBaseId > 0)
            <input type="hidden" name="knowledge_base_id" value="{{ $knowledgeBaseId }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif>
        @endif
        @if($articleId > 0)
            <input type="hidden" name="article_id" value="{{ $articleId }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif>
        @endif
        ...
        @if($sourceType === 'article')
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div>
                    ...
                    <textarea name="selected_article_text" rows="4" data-knowledge-correction-selected-text class="{{ $inputClass }}" placeholder="{{ __('admin.knowledge_corrections.placeholder_selected_article_text') }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif></textarea>
                ...
                <div>
                    ...
                    <select name="knowledge_base_id" class="{{ $inputClass }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif>
        ...
        <div>
            ...
            <textarea name="error_description" rows="4" required class="{{ $inputClass }}" placeholder="{{ __('admin.knowledge_corrections.placeholder_error_description') }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif>{{ old('error_description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
            <div>
                ...
                <select name="ai_model_id" class="{{ $inputClass }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif>
            ...
            <button type="submit" class="inline-flex h-10 items-center justify-center rounded-md border border-transparent px-4 text-sm font-medium text-white {{ $toneClasses['button'] }}" @if($externalFormId !== '') form="{{ $externalFormId }}" @endif>
        ...
        @push('scripts')
            <script>
                document.addEventListener('click', function (event) {
                    ...
                    const card = button.closest('[data-knowledge-correction-card]');
                    const textarea = card?.querySelector('[data-knowledge-correction-selected-text]');
                    if (!textarea) {
                        return;
                    }
